...
to setup
  ...
  ask one-patch [sprout-cows initial-population [set old false set shape "cow" set color white set size 4 disperse]]
end

to reproduce
  if (count cows) > 0
  [
    ask cows [ set old true ]
    set last-count count cows
    ask one-of cows [ hatch-cows how-many-to-hatch  [ disperse ] ]  ;Ok, Not too realistic to have one cow do all the reproductive work ... but easier to code.
    ask cows with [old] [die]
    do-plotting
  ]
end
...
"" ""
PENS
"pen-0" 1.0 0 -7500403 true "" "plotxy last-count count cows"

...
to setup-nest
  ...
  set nest-scent 16 - distancexy 0 0
end
...
to go  if not any? patches with [pcolor = green] [stop]
  ask turtles
  [ifelse color = red
    [
      ;; print "look for food"
...
to look-for-food
  ;;ifelse (chemical >= 0.05) and (chemical < 2)
   uphill-chemical
  ;;[ifelse coin-flip? [right random max-turn-angle] [left random max-turn-angle]] ;if coin-flip? = true, turn right
...
to uphill-nest-scent  ;; turtle procedure
  ...
  let scent-right nest-scent-at-angle  60
  let scent-left  nest-scent-at-angle -60
  if (scent-right > scent-ahead) or (scent-left > scent-ahead)
  [ ifelse scent-right > scent-left
    [ rt 45
      ;; print "scent right"
